# Demo IoT PCCC bằng C++

Toàn bộ mã nguồn của project dùng **Arduino C++**. ESP32 chạy trên Wokwi,
đọc cảm biến, điều khiển LED/còi/LCD và giao tiếp qua Wi-Fi + MQTT.
MQTTX hoặc MQTT Explorer chỉ là công cụ quan sát bản tin, tương tự Serial Monitor;
project không chứa Python, C# hay dashboard tự viết.
...
- Khi cả ba điều kiện trở lại bình thường, còi tự tắt ở lần đọc tiếp theo.
...
8. `mqttCallback()` nhận TEST hoặc RESET từ MQTTX.
